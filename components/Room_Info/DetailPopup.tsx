...
const DetailPopup = (props: DetailPopupProps) => {
  ...
  useEffect(() => {
    if (confirmRoom) {
      // เมื่อเปิด Popup: ห้ามเลื่อนหน้าจอหลัก
      document.body.style.overflow = "hidden";
    }

    return () => {
      // เมื่อปิด Popup (Cleanup): กลับมาเลื่อนได้ปกติ
          ...
          <div className="p-4 max-h-[60dvh] overflow-y-auto custom-scrollbar rounded-xl">
            ...
            <div className="grid grid-cols-2 gap-y-2 text-[12px] md:text-[14px]">
              ...
              <p>
                ...
                {confirmRoom.price} THB/M
              </p>
              <p>
                <span className="font-bold text-black">Dorm Zone:</span>{" "}
                {formRoom.dorm.name}
              </p>
              <p>
                <span className="font-bold text-black">Capacity:</span>{" "}
                {confirmRoom.capacity}
              </p>
              <p>
                <span className="font-bold text-black">Floor:</span>{" "}
                {confirmRoom.floor}
              </p>
              ...
              <p>
                ...
                {Math.min(confirmRoom.currentOccupancy, confirmRoom.capacity)} /{" "}
                {confirmRoom.capacity}
              </p>
              <p>
              ...
              <p>
                ...
                {confirmRoom.roomType}
              </p>
            </div>

            <div className="flex flex-col gap-1 mt-2 mb-2 text-[12px] md:text-[14px]">
              <span className="font-bold text-black">
                Roommates' Faculty / คณะของผู้อยู่:
              </span>
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <div className="w-16 h-1 bg-gray-100 rounded-full mt-1 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${calculateMatch(confirmRoom.lifestyleConfig)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  {(confirmRoom.lifestyleConfig || []).map((tag: string) => {
                    ...
                    const config =
                      DORM_LABELS.LIFESTYLE[
                        tag as keyof typeof DORM_LABELS.LIFESTYLE
                      ];
            ...
            <div className="flex flex-col items-center w-full mt-4 gap-2">
              ...
              {isCharterBroken && (
                <p className="text-red-500 text-[12px] font-medium bg-red-50 px-3 py-1 rounded-lg border border-red-100 mb-1">
                  ⚠️ ห้องนี้มีผู้พักแล้ว ไม่สามารถจองแบบเหมาได้ - This room is
                  already occupied and cannot be booked as a whole.
                </p>
              )}

              {isRoomFull && (
                <p className="text-red-500 text-[12px] font-medium bg-red-50 px-3 py-1 rounded-lg border border-red-100 mb-1">
                  ⚠️ ขออภัย ห้องพักนี้เต็มแล้ว - Sorry, this room is fully
                  booked.
                </p>
              )}
