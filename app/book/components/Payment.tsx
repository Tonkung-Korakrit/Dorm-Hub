...
import { MdCheckCircle, MdTimer, MdQrCodeScanner, MdEmail, MdErrorOutline, MdCloudUpload } from "react-icons/md";
      ...
      <div className="mt-8 space-y-4">
        ...
        <div className="space-y-6">
          <div
            ...
            className={`group cursor-pointer border-2 border-dashed rounded-3xl p-8 flex flex-col items-center transition-all
            ${selectedFile ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-[#126A31] hover:bg-green-50'}`}
          >
            <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileChange} accept="image/*" />
            <MdCloudUpload size={40} className={selectedFile ? 'text-emerald-500' : 'text-gray-300 group-hover:text-[#126A31]'} />
            <p className="mt-3 font-bold text-sm text-gray-600">
              {selectedFile ? selectedFile.name : 'Upload Payment Slip / แนบรูปสลิป'}
            </p>
