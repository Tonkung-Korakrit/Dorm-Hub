...
export async function GET() {
  try {
    const users = await prisma.cus_users.findMany({
      select: {
        id: true,
        citizenType: true,
        citizenNumber: true,
...
export async function POST(req) {
  try {
    ...
    if (filteredBody.studentId) {
      const existingUser = await prisma.cus_users.findUnique({
        where: { studentId: String(filteredBody.studentId) },
      });
    ...
    //   where: String(filteredBody.studentId),
    //   update: data,
    //   // create: filteredBody,
    // });
