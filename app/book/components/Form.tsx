...
import { BookRoomFormProps } from "@/types/booking";
import { BookingSummary } from "./BookingSummary";
import { PaymentPage } from "./Payment";

export default function BookRoomForm({ user }: BookRoomFormProps) {
  const [step, setStep] = useState<number>(1);
  const { formResident, setFormResident, formRoom, setFormRoom } = useBooking();
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-transparent overflow-hidden">
      ...
      <div className="flex-1 flex justify-center overflow-hidden">
        ...
        >
          <div className="w-full max-w-4xl overflow-y-auto pl-2 pr-0 pb-1 md:px-8 custom-scrollbar">
            {step === 1 && <StudentInfoForm setStep={setStep} />}
            {step === 2 && <ProfileStep setStep={setStep} />}
            {step === 3 && <VehicleStep setStep={setStep} />}
            {step === 4 && <DormSelector setStep={setStep} />}
            {step === 5 && <ZoneSelector setStep={setStep} />}
            {step === 6 && <RoomGridSelection setStep={setStep} />}
            {step === 7 && <BookingSummary setStep={setStep} />}
            {step === 8 && <PaymentPage />}
          </div>
        </div>
      </div>
